...
class TF4CTR_test(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="TF4CTR_test",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 c=0.8,
                 ...
                 **kwargs):
        super(TF4CTR_test, self).__init__(feature_map,
                                     model_id=model_id,
                                     gpu=gpu,
        ...
        for epoch in range(epochs):
            ...
            if epoch == 0:
                torch.save(self.grad, os.path.abspath(
                    os.path.join(self.model_dir, self.model_id + ".logloss_grad_norm_{}_epoch".format(self._epoch_index))))
    ...
    def train_step(self, batch_data):
        ...
        return_dict['y_easy'].retain_grad()
        return_dict['y_hard'].retain_grad()
        return_dict['y_pred'].retain_grad()
        loss.backward()
        if self.has_gradients():
            # Calculate gradient norms
            grad_norm_y_pred = return_dict["y_pred"].grad.norm()
            grad_norm_y_easy = return_dict["y_easy"].grad.norm()
            grad_norm_y_hard = return_dict["y_hard"].grad.norm()
            self.grad.append([grad_norm_y_pred, grad_norm_y_easy, grad_norm_y_hard])
