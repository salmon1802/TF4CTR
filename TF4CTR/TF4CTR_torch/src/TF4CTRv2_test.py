...
# Full TF4CTR
class TF4CTRv2_test(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="TF4CTRv2_test",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 c=0.8,
                 ...
                 **kwargs):
        super(TF4CTRv2_test, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        self.val_res_pos = []
        self.val_res_neg = []
        self.all_val_res_pos = []
        self.all_val_res_neg = []
    ...
    def forward(self, inputs):
        ...
        if self.training == False:
            label = self.get_labels(inputs)
            self.val_res_pos.append(y_pred[label == 1])
            self.val_res_neg.append(y_pred[label == 0])
        return return_dict
    ...
    def evaluate(self, data_generator, metrics=None):
        ...
        with torch.no_grad():
            ...
            self.all_val_res_pos.append(torch.cat(self.val_res_pos))
            self.val_res_pos.clear()
            self.all_val_res_neg.append(torch.cat(self.val_res_neg))
            self.val_res_neg.clear()
            torch.save(self.all_val_res_pos, os.path.abspath(
                os.path.join(self.model_dir, self.model_id + ".pos_all")))
            torch.save(self.all_val_res_neg, os.path.abspath(
                os.path.join(self.model_dir, self.model_id + ".neg_all")))
            return val_logs
