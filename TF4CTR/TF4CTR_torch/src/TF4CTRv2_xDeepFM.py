...
# Full TF4CTR
class TF4CTRv2_xDeepFM(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="TF4CTRv2_xDeepFM",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 c=0.8,
                 ...
                 **kwargs):
        super(TF4CTRv2_xDeepFM, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        self.lr_layer = LogisticRegression(feature_map, use_bias=False)  # 逻辑回归
        self.easy_encoder = CompressedInteractionNet(feature_map.num_fields, cin_hidden_units, output_dim=hard_hidden_units[-1])
        ...
        if DFM == 'WSF':
            ...
            self.DFM = VF(sim_encoder_dim=hard_hidden_units[-1], com_encoder_dim=hard_hidden_units[-1])
        elif DFM == 'CF':
            self.DFM = CF(sim_encoder_dim=hard_hidden_units[-1], com_encoder_dim=hard_hidden_units[-1])
        elif DFM == 'MoEF':
            self.DFM = MoEF(sim_encoder_dim=hard_hidden_units[-1], com_encoder_dim=hard_hidden_units[-1])
        else:
    ...
    def forward(self, inputs):
        ...
        y_pred = self.output_activation(y_pred + lr_logit)
...
class SER(nn.Module):
    ...
    def forward(self, X):
        easy_out = self.easy_sample_embedding(X)
