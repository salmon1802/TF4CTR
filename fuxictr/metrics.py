...
from sklearn.metrics import roc_auc_score, log_loss, accuracy_score, mean_squared_error, precision_score, recall_score, f1_score, confusion_matrix
import numpy as np
import pandas as pd
...
def evaluate_metrics(y_true, y_pred, metrics, group_id=None):
    ...
    for metric in metrics:
        if metric in ['logloss', 'binary_crossentropy']:
            ...
            spe = tn / (tn + fp)
            return_dict[metric] = spe
        elif metric in ['accuracy']:
            return_dict[metric] = accuracy_score(y_true, (y_pred >= 0.5).astype(int))
        elif metric in ['precision']:
            return_dict[metric] = precision_score(y_true, (y_pred >= 0.5).astype(int))
        elif metric in ['recall', 'SEN']:
            return_dict[metric] = recall_score(y_true, (y_pred >= 0.5).astype(int))
        elif metric in ['f1_score']:
            return_dict[metric] = f1_score(y_true, (y_pred >= 0.5).astype(int))
